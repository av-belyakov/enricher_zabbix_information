...
8. [ ] Запуск цикла, прослушивающего канал временного интервала или сигнала по расписанию (например внедрить расписание) для запуска основного обработчика.
       Использовать пакет **gocron**
...
13. [ ] Выполнить валидацию доменного имени с помощью пакета net/url.

14. [ ] Доменное имя хостов, подходящих по условию, отправить для обогащения в виде запроса к DNS серверу. Обработать ошибки которые могут возникнуть при резолве доменного имени, такие как ip адрес не найден (может быть не верный домен), DNS сервер не доступен и т.д. Типы ошибок вроде есть в net пакете, тип DNSError. Нужно будет использовать штатные средства Go для DNS запросов. Количество workers для таких запросов ограничить определенным числом.

15. [ ] После получения ответа от DNS сервера, выполняется запрос к NetBox для проверки ip адреса на вхождение его в определённый сетевой диапазон. Если ip адрес входит в определённый сетевой диапазон, то нужно добавить в Zabbix, для данного хоста, тег формата Name :'СОА', Value:'id СОА'.

16. [ ] Думаю, что после обработки списка хостов, временное хранилище нужно очистить, что бы не засорять память. Значит надо предусмотреть очистку хранилища.

17. [ ] Наверное стоит сделать REST API через который можно будет выполнять следующие действия:

- принудительный запуск всего вышеперечисленног перечня коман по получению списка хостов, преобразования доменных имён в ip адреса с помощью DNS, поиска вхождения ip адреса в подсеть и т.д.
- получение информации о ранее выполненной задачи, её успешность, количество обработаных, найденых хостов и т.д.
- когда **GO_ENRICHERZI_MAIN** development или test, можно сделать профелирование памяти.
